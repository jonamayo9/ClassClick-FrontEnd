...
function TeacherProfileInner() {
  ...
  useEffect(() => {
    apiService.get<{ url?: string }>('/api/teacher/profile/photo').then((r) => { if (r?.url) setCurrentPhotoUrl(r.url) }).catch(() => {})
  }, [])

  const photoMutation = useMutation({
    mutationFn: (fd: FormData) => apiService.postForm<{ url?: string }>(`/api/teacher/profile/photo`, fd),
    onSuccess: (data) => { setCurrentPhotoUrl(data?.url ?? null); setSelectedFile(null); toast('Foto actualizada.') },
    onError: () => toast('Error al subir foto.', 'error'),
  })

  const passwordMutation = useMutation({
    mutationFn: () => apiService.put(`/api/teacher/profile/password`, { currentPassword, newPassword, confirmNewPassword: confirmPassword }),
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-800 p-5 text-white sm:p-6">
        ...
        <p className="mt-1 text-sm text-emerald-200">Datos de usuario y preferencias</p>
      </div>
          ...
          <div className="space-y-3 flex-1">
            ...
            {previewUrl ? (
              <div className="flex gap-2">
                <Button loading={photoMutation.isPending} onClick={handleUpload}
                  className="bg-emerald-600 text-white hover:bg-emerald-700">Guardar foto</Button>
      ...
      <Card className="p-5 sm:p-6 space-y-4">
        ...
        <div className="space-y-3 max-w-md">
          ...
          <Button onClick={() => passwordMutation.mutate()} loading={passwordMutation.isPending}
            ...
            className="bg-emerald-600 text-white hover:bg-emerald-700">Cambiar contraseña</Button>
        </div>
      </Card>
...
export default function TeacherProfilePage() { return <ToastProvider><TeacherProfileInner /></ToastProvider> }
